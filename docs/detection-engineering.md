...
|---|---|---|---|
| Internal Nmap ACK scan | Suricata via Logstash | KQL rule against ECS-normalized Suricata alert indices | `T1046` Network Service Discovery |
...
The Security rule is scoped to internal-to-internal traffic:
...
Rather than suppressing the signature entirely, the Elastic Security rule was narrowed to internal source and destination CIDR ranges. This preserves useful internal scan coverage while preventing unrelated Internet return traffic from being promoted into Security alerts.
...
Both rules have been exercised using controlled lab traffic and confirmed on the Elastic Security Alerts page.

![Elastic Security alerts](images/kibana/Sec_Alerts.png)

*Elastic Security showing the internal Nmap ACK-scan detection and Linux SSH password-guessing correlation rule operating together.*
